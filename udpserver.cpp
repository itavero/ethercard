...
void EtherCard::udpServerListen(UdpServerCallback callback, uint8_t address[4], uint16_t port, bool allAddresses) {
	if(numListeners < UDPSERVER_MAXLISTENERS)
	{
		listeners[numListeners] = (UdpServerListener){callback,
			{address[0], address[1], address[2], address[3]},
			port, true, allAddresses};
		numListeners++;
	}
}
...
void EtherCard::udpServerPauseListen(uint8_t address[4], uint16_t port) {
...
void EtherCard::udpServerResumeListen(uint8_t address[4], uint16_t port) {
	for(int i = 0; i < numListeners; i++)
	{
		if(listeners[i].port == port && compareAddresses(listeners[i].address, address)) {
